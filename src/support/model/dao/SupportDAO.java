...
public class SupportDAO {
	...
	public Support selectFirstSupport(Connection conn, int firstRank) {
		String query = "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY (NOW_SUPPORT/NEED_SUPPORT)*100 DESC) AS NUM, NOTICE_NO, NOTICE_TITLE, PIC_NAME, NOTICE_CONTENTS, VIEWS, SUPPORT_HUMAN, NEED_SUPPORT, NOW_SUPPORT, PIC_PATH, LEVELCHECK FROM NOTICE WHERE LEVELCHECK = 'Y') WHERE NUM = ?";
	...
	public Support selectSecondSupport(Connection conn, int secondRank) {
		String query = "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY (NOW_SUPPORT/NEED_SUPPORT)*100 DESC) AS NUM, NOTICE_NO, NOTICE_TITLE, PIC_NAME, NOTICE_CONTENTS, VIEWS, SUPPORT_HUMAN, NEED_SUPPORT, NOW_SUPPORT, PIC_PATH FROM NOTICE WHERE LEVELCHECK = 'Y') WHERE NUM = ?";
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		Support support = null;
